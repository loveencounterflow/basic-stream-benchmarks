...
(function() {
  var $input, $logger, $map, $on_end, $output, $random, $sequence, $split, $stop_time, $stringify, $utf8, CND, FS, O, PATH, badge, count, debug, echo, format_float, format_integer, help, id, info, input_stream, mkdirp, new_numeral, output_stream, parseCsv, paths, pipeline, prop, pull, push, rpr, t0, t1, through, through2, to_pull, urge, values, warn, whisper;
...
;
  ...
  $input = function() {
    return to_pull.source(input_stream);
  };

  $output = function() {
    return to_pull.sink(output_stream, function(error) {
      var dts, ips;
      if (error != null) {
        throw error;
      }
